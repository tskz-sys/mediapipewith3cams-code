...
# ==========================================================
# Ball false-positive gating / tracking (重要)
#
# 症状:
#   - cam3(右)のコート外・壁際の静止ボールに反応し、cam1の本物ボールと
#     2視点で三角測量されてしまう。
#
# 対策:
#   - 各カメラのball候補から「プレー領域っぽい候補」を1つ選ぶ（または捨てる）。
#   - cam3は右端をカットしやすい（静止ボールが出やすい領域）。
#   - 人bbox周りをフォーカス領域として、そこから遠いball候補を落とす。
#   - 前フレームのball位置と比べて、極端なジャンプを落とす（2Dピクセル）。
#
# NOTE:
#   - これは3D triangulation前に効かせるのがポイント。

BALL_GATE_ENABLE = True
# people union bbox を上下左右にどれだけ広げるか（bboxサイズに対する倍率）
BALL_GATE_EXPAND = 1.8
# cam3(右)の「右端を切る」割合（0-1）。静止ボールが右端に出る場合に有効。
BALL_GATE_CAM3_RIGHT_CUT = 0.86
# ball 2D center が前フレームからこのpx以上飛んだら無効（偽ボールの瞬間置換対策）
BALL_GATE_MAX_JUMP_PX = 250.0
# 初期（prevが無い）で focus bbox からこのpx以上離れてたら無効
BALL_GATE_OUTSIDE_FOCUS_PX = 140.0
...
def process_person_yolo(
    ...
    D: np.ndarray,
    max_p: int,
    min_visible_joints: Optional[int],
    min_visible_conf: float,
):
    H, W = img.shape[:2]
        ...
        if mp_kps is not None:
            ...
            for i in range(len(mp_kps)):
                ...
                if kx < margin or kx > W - margin or ky < margin or ky > H - margin:
                    mp_confs[i] = 0.0
            if min_visible_joints is not None and min_visible_joints > 0:
                visible = int(np.sum(mp_confs >= min_visible_conf))
                if visible < min_visible_joints:
                    continue
...
def _people_union_bbox(people: List[dict]) -> Optional[Tuple[float, float, float, float]]:
...
def _expand_bbox(b: Tuple[float, float, float, float], expand: float, W: int, H: int) -> Tuple[float, float, float, float]:
...
def _point_bbox_distance(px: float, py: float, b: Tuple[float, float, float, float]) -> float:
...
def select_ball_candidate(
    balls: List[dict],
    people: List[dict],
    prev_center_raw: Optional[np.ndarray],
    frame_shape: Tuple[int, int, int],
    cam_idx: int,
    expand: float,
    cam3_right_cut: float,
    max_jump_px: float,
    outside_focus_px: float,
) -> List[dict]:
    """Pick 0..1 ball candidate per camera to reduce static false positives.

    Key behavior (hard reject):
      - If a focus bbox (union of people bboxes) exists, any ball candidate whose
        center is farther than outside_focus_px from the focus bbox is excluded
        *even when prev_center_raw exists*. This prevents a false ball from being
        "locked" by temporal continuity.

    Returns a list (empty or [ball_dict]).
    """
    if not balls:
        return []
    ...
    # 1) cam3 right-cut (static ball often appears far right)
    ...
    # 2) focus region from people union bbox (expanded)
    ...
    # 3) scoring: prefer close to prev; also prefer near focus (weak)
    best = None
    best_score = -1e18
    for b in balls:
        ...
        # --- HARD REJECT: outside focus (prevents false-lock) ---
        d_focus = None
        if hard_focus:
            d_focus = _point_bbox_distance(cx, cy, focus)  # type: ignore[arg-type]
    ...
    # 4) hard gates (jump + focus; focus is redundant but kept as a safety net)
...
def solve_3d_joints(
    ...
    # Choose the best 2-camera pair per joint using confidence and parallax.
    ...
    for j in range(33):
        ...
        for cam_idx in cams:
            ...
            if not np.isfinite(conf) or conf < min_joint_conf:
                continue
            valid.append((cam_idx, conf, p["kps_norm"][j]))

        if len(valid) < 2:
            continue

        best = None
        for a in range(len(valid) - 1):
            cam_a, conf_a, pt_a = valid[a]
            for b in range(a + 1, len(valid)):
                cam_b, conf_b, pt_b = valid[b]
                X, err = triangulate_DLT(
                    [Ps[cam_a], Ps[cam_b]],
                    [pt_a, pt_b],
                )
                if np.linalg.norm(X) >= 50.0:
                    continue
                if max_triang_err is not None and err > max_triang_err:
                    continue

                C1 = _camera_center_from_P(Ps[cam_a])
                C2 = _camera_center_from_P(Ps[cam_b])
                par_deg = _parallax_deg(C1, C2, X)
                if parallax_min_deg is not None and parallax_min_deg > 0:
                    if par_deg < parallax_min_deg:
                        continue

                score = conf_a + conf_b
                if best is None:
                    best = (score, par_deg, err, X)
                else:
                    best_score, best_par, best_err, _ = best
                    if (
                        score > best_score
                        or (score == best_score and par_deg > best_par)
                        or (score == best_score and par_deg == best_par and err < best_err)
                    ):
                        best = (score, par_deg, err, X)
...
def _drop_bone_outliers(
    df: pd.DataFrame,
    bones: List[Tuple[str, str, str]],
    ratio_max: Optional[float],
    abs_max: Optional[float],
) -> pd.DataFrame:
    if ratio_max is None and abs_max is None:
        return df
    ratio_max_val = float(ratio_max) if ratio_max is not None and ratio_max > 0 else float("inf")
    abs_max_val = float(abs_max) if abs_max is not None and abs_max > 0 else None

    to_drop = []
    for pid in sorted(df['person_id'].unique()):
        if int(pid) < 0:
            continue
        df_pid = df[df['person_id'] == pid]
        for ja, jb, drop_j in bones:
            a = df_pid[df_pid['joint'] == ja]
            b = df_pid[df_pid['joint'] == jb]
            if a.empty or b.empty:
                continue
            merged = a.merge(b, on=['frame', 'person_id'], suffixes=('_a', '_b'))
            if merged.empty:
                continue
            vec_a = merged[['X_a', 'Y_a', 'Z_a']].to_numpy(dtype=float)
            vec_b = merged[['X_b', 'Y_b', 'Z_b']].to_numpy(dtype=float)
            lengths = np.linalg.norm(vec_a - vec_b, axis=1)
            good = np.isfinite(lengths) & (lengths > 0.0)
            if abs_max_val is not None:
                good &= lengths <= abs_max_val
            if not np.any(good):
                continue
            med = float(np.median(lengths[good]))
            if med <= 1e-6:
                continue
            too_long = lengths > med * ratio_max_val
            if abs_max_val is not None:
                too_long = too_long | (lengths > abs_max_val)
            if not np.any(too_long):
                continue
            bad_frames = merged.loc[too_long, 'frame'].astype(int).tolist()
            to_drop.extend((int(pid), int(f), drop_j) for f in bad_frames)

    if not to_drop:
        return df
    drop_df = pd.DataFrame(to_drop, columns=['person_id', 'frame', 'joint']).drop_duplicates()
    df_out = df.merge(drop_df.assign(drop=1), on=['person_id', 'frame', 'joint'], how='left')
    mask = df_out['drop'] == 1
    df_out.loc[mask, ['X', 'Y', 'Z']] = np.nan
    return df_out.drop(columns=['drop'])


def _apply_bidirectional_ema(values: np.ndarray, alpha: Optional[float]) -> np.ndarray:
    if alpha is None or alpha <= 0 or alpha >= 1:
        return values
    out = values.copy()
    for axis in range(out.shape[1]):
        series = pd.Series(out[:, axis])
        fwd = series.ewm(alpha=alpha, adjust=False).mean()
        bwd = series[::-1].ewm(alpha=alpha, adjust=False).mean()[::-1]
        out[:, axis] = (fwd + bwd) / 2.0
    return out
...
def smooth_csv_like(
    ...
    foot_ratio_max: Optional[float] = 2.2,
    foot_abs_max: Optional[float] = None,
    ema_alpha: Optional[float] = None,
):
    df = pd.read_csv(input_csv)
    df = df.sort_values(['person_id', 'joint', 'frame'])

    foot_ratio = foot_ratio_max if foot_ratio_max is not None and foot_ratio_max > 0 else None
    foot_abs = foot_abs_max if foot_abs_max is not None and foot_abs_max > 0 else None
    if foot_ratio is not None or foot_abs is not None:
        foot_bones = [
            ("left_knee", "left_ankle", "left_ankle"),
            ("right_knee", "right_ankle", "right_ankle"),
            ("left_ankle", "left_heel", "left_heel"),
            ("left_ankle", "left_foot_index", "left_foot_index"),
            ("right_ankle", "right_heel", "right_heel"),
            ("right_ankle", "right_foot_index", "right_foot_index"),
        ]
        df = _drop_bone_outliers(df, foot_bones, ratio_max=foot_ratio, abs_max=foot_abs)
    ...
    for (pid, joint), group in df.groupby(['person_id', 'joint']):
        ...
        g[['X', 'Y', 'Z']] = g[['X', 'Y', 'Z']].interpolate(
            method='linear',
            limit=interpolate_limit,
            limit_direction='both',
        )
        g[['X', 'Y', 'Z']] = g[['X', 'Y', 'Z']].rolling(
            window=window,
            center=True,
            min_periods=1,
        ).mean()
        if ema_alpha is not None and ema_alpha > 0:
            ...
            coords = _apply_bidirectional_ema(coords, ema_alpha)
            g[['X', 'Y', 'Z']] = coords
...
def filter_pose_bone_lengths(
    ...
    if ratio_max is None and ratio_min is None and abs_max is None:
        df.to_csv(output_csv, index=False)
        return

    ratio_max = float(ratio_max) if ratio_max is not None else float("inf")
    ...
    for pid in sorted(df['person_id'].unique()):
        ...
        for ja, jb, drop_j in bones:
            ...
            if abs_max is not None and abs_max > 0:
                good &= lengths <= abs_max
            if not np.any(good):
                continue
            med = float(np.median(lengths[good]))
            if med <= 1e-6:
                continue
            too_long = lengths > med * ratio_max
            too_short = lengths < med * ratio_min
            if abs_max is not None and abs_max > 0:
                too_long = too_long | (lengths > abs_max)
            bad = too_long | too_short
            if not np.any(bad):
                continue
            bad_frames = merged.loc[bad, 'frame'].astype(int).tolist()
            to_drop.extend((int(pid), int(f), drop_j) for f in bad_frames)
...
def gate_by_reliable_cams(
    kps_3d: np.ndarray,
    persons: List[Optional[dict]],
    cams: List[int],
    Ps: List[np.ndarray],
    min_joint_conf: float,
    reliable_cams_min: Optional[int],
    reliable_reproj_thresh: Optional[float],
) -> np.ndarray:
    if reliable_cams_min is None or reliable_cams_min <= 0:
        return kps_3d
    cams_to_check = cams
    gated = kps_3d.copy()
    for j in range(gated.shape[0]):
        ...
        for cam_idx in cams_to_check:
            ...
            if not np.isfinite(conf) or conf < min_joint_conf:
                continue
            if reliable_reproj_thresh is not None and reliable_reproj_thresh > 0:
                proj = _project_normalized(Ps[cam_idx], gated[j])
                if proj is None:
                    continue
                err = float(np.linalg.norm(p["kps_norm"][j] - proj))
                if err > reliable_reproj_thresh:
                    continue
            reliable += 1
        if reliable < reliable_cams_min:
...
class RunOutputs:
    ...
    bone_filtered_csv: Optional[str]
...
def run_one_triplet(
    ...
    foot_ratio_max: Optional[float],
    foot_abs_max: Optional[float],
    smooth_ema_alpha: Optional[float],
    reassign_ids_by_ball: bool,
    reproj_err_thresh: Optional[float],
    jump_center_thresh: Optional[float],
    bone_ratio_max: Optional[float],
    bone_ratio_min: Optional[float],
    bone_abs_max: Optional[float],
    reliable_cams_min: Optional[int],
    reliable_reproj_thresh: Optional[float],
    min_visible_joints: Optional[int],
    pair_cost_max: float,
    ...
    min_cams: int,
    ball_gate_enable: bool,
    ball_gate_expand: float,
    ball_gate_cam3_right_cut: float,
    ball_gate_max_jump_px: float,
    ball_gate_outside_focus_px: float,
) -> RunOutputs:
    os.makedirs(out_dir, exist_ok=True)
    ...
    out_bone_csv = os.path.join(out_dir, f"{out_stem}_bone_filtered.csv")
    ...
    # per-camera 2D ball tracker (to avoid static false positives)
    prev_ball_center_raw: List[Optional[np.ndarray]] = [None, None, None]
    ...
    for i in tqdm(range(total_frames), desc=out_stem):
        ...
        for cam_i, f in enumerate(frames):
            ...
            p = process_person_yolo(
                ...
                min_visible_conf=min_joint_conf,
            )
            ppl_lists.append(p)
            b_all = detect_ball_yolo(f, yolo_det, K, D)
            if ball_gate_enable:
                b_sel = select_ball_candidate(
                    balls=b_all,
                    people=p,
                    prev_center_raw=prev_ball_center_raw[cam_i],
                    frame_shape=f.shape,
                    cam_idx=cam_i,
                    expand=ball_gate_expand,
                    cam3_right_cut=ball_gate_cam3_right_cut,
                    max_jump_px=ball_gate_max_jump_px,
                    outside_focus_px=ball_gate_outside_focus_px,
                )
                if len(b_sel) > 0:
        ...
        for match in matched_results:
            ...
            kps_3d = gate_by_reliable_cams(
                kps_3d,
                persons,
                active_cams,
                Ps,
                min_joint_conf=min_joint_conf,
                reliable_cams_min=reliable_cams_min,
                reliable_reproj_thresh=reliable_reproj_thresh,
            )
    ...
    bone_csv_path: Optional[str] = None
    ...
    if bone_ratio_max is not None or bone_ratio_min is not None or bone_abs_max is not None:
        filter_pose_bone_lengths(
            smoothing_input_csv,
            out_bone_csv,
            ratio_max=bone_ratio_max,
            ratio_min=bone_ratio_min,
            abs_max=bone_abs_max,
        )
        bone_csv_path = out_bone_csv
        smoothing_input_csv = out_bone_csv
    ...
    if run_smoothing:
        ...
        smooth_csv_like(
            ...
            foot_ratio_max=foot_ratio_max,
            foot_abs_max=foot_abs_max,
            ema_alpha=smooth_ema_alpha,
        )
    ...
    return RunOutputs(
        ...
        bone_filtered_csv=bone_csv_path,
...
def main():
    ...
    ap.add_argument("--foot_ratio_max", type=float, default=2.2, help="smooth step: drop foot/ankle if bone length exceeds ratio * median (0 to disable)")
    ap.add_argument("--foot_abs_max", type=float, default=None, help="smooth step: drop foot/ankle if bone length exceeds absolute value")
    ap.add_argument("--smooth_ema_alpha", type=float, default=0.35, help="smooth step: bidirectional EMA alpha (0 to disable)")
    ...
    ap.add_argument("--reliable_cams_min", type=int, default=None, help="set joints to NaN if reliable cameras below this")
    ap.add_argument("--reliable_reproj_thresh", type=float, default=None, help="reliable cam requires reprojection error <= this (normalized)")
    ap.add_argument("--bone_ratio_max", type=float, default=None, help="drop distal joints if bone length exceeds ratio * median")
    ap.add_argument("--bone_ratio_min", type=float, default=None, help="drop distal joints if bone length below ratio * median")
    ap.add_argument("--bone_abs_max", type=float, default=None, help="drop distal joints if bone length exceeds this absolute value")
    ...
    ap.add_argument("--min_visible_joints", type=int, default=MIN_VISIBLE_JOINTS, help="drop persons with too few visible joints")

    # --- Ball gating (static false ball対策) ---
    ap.add_argument("--ball_gate_disable", action="store_true", help="disable per-camera ball gating/tracking")
    ap.add_argument("--ball_gate_expand", type=float, default=BALL_GATE_EXPAND, help="expand ratio for people-union bbox when gating balls")
    ap.add_argument("--ball_gate_cam3_right_cut", type=float, default=BALL_GATE_CAM3_RIGHT_CUT, help="cam3: drop balls with cx > W*cut (0-1). set 1.0 to disable")
    ap.add_argument("--ball_gate_max_jump_px", type=float, default=BALL_GATE_MAX_JUMP_PX, help="reject ball 2D jumps larger than this (px/frame)")
    ap.add_argument("--ball_gate_outside_focus_px", type=float, default=BALL_GATE_OUTSIDE_FOCUS_PX, help="if no prev ball, drop balls farther than this from focus bbox (px)")
    ...
    for game, match, v1, v2, v3 in triplets:
        ...
        outs = run_one_triplet(
            ...
            bone_ratio_max=args.bone_ratio_max,
            bone_ratio_min=args.bone_ratio_min,
            bone_abs_max=args.bone_abs_max,
            reliable_cams_min=args.reliable_cams_min,
            reliable_reproj_thresh=args.reliable_reproj_thresh,
        ...
        if outs.bone_filtered_csv:
            print(f"  -> boneflt : {outs.bone_filtered_csv}")
