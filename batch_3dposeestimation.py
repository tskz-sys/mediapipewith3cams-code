...
def load_params_BR(npz_path: str, v1: str, v2: str, v3: str, invert_both: bool = False):
...
def solve_3d_joints(
    ...
    cam_centers = [_camera_center_from_P(Ps[i]) for i in range(len(Ps))]
    for j in range(33):
        avail = []
        for cam_idx in cams:
            p = persons[cam_idx]
        ...
        for i in range(len(avail)):
            for k in range(i + 1, len(avail)):
                c1, c2 = avail[i], avail[k]
                X, tri_err = triangulate_DLT(
                    [Ps[c1], Ps[c2]],
                    [persons[c1]["kps_norm"][j], persons[c2]["kps_norm"][j]],
                )
                if max_triang_err is not None and tri_err > max_triang_err:
                    continue
                if np.linalg.norm(X) >= 50.0:
                    continue
                if parallax_min_deg is not None and parallax_min_deg > 0:
                    ang = _parallax_deg(cam_centers[c1], cam_centers[c2], X)
                    if ang < parallax_min_deg:
                        continue

                errs = []
                for cam_idx in avail:
                    proj = _project_normalized(Ps[cam_idx], X)
                    if proj is None:
                        continue
                    obs = persons[cam_idx]["kps_norm"][j]
                    errs.append(float(np.linalg.norm(obs - proj)))
                if not errs:
                    continue
...
def run_one_triplet(
    ...
    cam_params_full, cam_rt = load_params_BR(
        calib_npz, video_left, video_center, video_right, invert_both=invert_both
    )
