...
def run_hybrid_one(mod, video1: str, video2: str, video3: str, npz: str, out_csv: str, out_video: str) -> None:
    """Run hybrid main() safely.

    IMPORTANT:
      The hybrid script's main() uses argparse (sys.argv). If we import the module
      and call main() directly, it would try to parse THIS wrapper's argv (e.g.
      --input_dir/--games/--matches) and crash.

      So we temporarily replace sys.argv with the hybrid CLI arguments.
    """
    v1 = normalize_wsl_path(video1)
    v2 = normalize_wsl_path(video2)
    v3 = normalize_wsl_path(video3)
    npz = normalize_wsl_path(npz)
    out_csv = normalize_wsl_path(out_csv)
    out_video = normalize_wsl_path(out_video)
    ...
    old_argv = sys.argv[:]
    try:
        prog = getattr(mod, '__file__', 'hybrid')
        sys.argv = [
            str(prog),
            '--video_left', v1,
            '--video_center', v2,
