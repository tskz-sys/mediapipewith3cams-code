...
# -*- coding: utf-8 -*-
# ==========================================================
# YOLO + MediaPipe Hybrid [Fixed Version]
#   - Fix: Added 'import subprocess'
#   - Fix: triangulate_DLT now returns (X, error) correctly
#   - Detection: YOLOv11x (Box) + MediaPipe (Pose)
...
# ★Pattern BR (逆変換) スイッチ★
# 骨格が浮く場合は False (OFF) に、それでダメなら True (ON) に
...
def main():
    ...
    print(f"=== Hybrid Batch (YOLO-Box + MediaPipe-Pose) ===")
    ...
    for match_id in TARGET_MATCHES:
        for clip_id in TARGET_CLIPS:
            ...
            # Output Files
            base_name = f"hybrid_mp_match{match_id}_clip{clip_id}"
